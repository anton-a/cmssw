				       ...
				       reco::GsfTrack gsftrack)
{
  ...
  if(debug)
    cout << "Entering ConvBremTrackFinder::runConvBremFinder\n";
  ...
  if(debug)
    cout << "ConvBremTrackFinder::runConvBremFinder - After getting the track collection\n";
  ...
  if(debug)
    cout << "ConvBremTrackFinder::runConvBremFinder - After getting inner/outer TSOS of the gsf track\n";
  ...
  if(debug)
    cout << "ConvBremTrackFinder::runConvBremFinder - GSFtrack: pt " << gsftrack.ptMode()
	 << " eta,phi " << gsftrack.eta() << ", " << gsftrack.phi() << endl;
  ...
  // mnow it is passed directly from the configuration

  // loop over the KF tracks 
  unsigned int i = 0;
  for(;tkit!=tkitend;++tkit,i++){
    if(debug)
      cout << " KFtrack: pt " << tkit->pt() 
	   << " eta,phi " << tkit->eta() << ", " << tkit->phi() << endl;

    if(!((TkColl[i]).quality(trackQuality_))) continue;
    ...
    if (eleSeedRef->ctfTrack().isNull()) ctfTkRef = gsftrack.gsfExtra()->MatchedCtfTrack();
    // CASE 2: the electron SEED has a reference to a ctf track 
    else ctfTkRef = eleSeedRef->ctfTrack();
    if (ctfTkRef.isNonnull()) {
      if(trackRef == ctfTkRef) {
	       if (debug) cout << " ** SAME REF SEED ** " << endl;
	       continue;
      }
    }
  ...
  //   cout << " ** dphi_presel  ** " << dphi_presel 
// 	 << " ** deta_presel ** " << deta_presel
// 	 << " tangent size  " <<  gsftrack.gsfExtra()->tangentsSize() << endl;

    for (unsigned int iTang = 0; iTang < gsftrack.gsfExtra()->tangentsSize(); iTang++) {

      // Replace the hotwired valies!!! (AA)
    ...
    if ( (minDPhiBremKF < 0.1 || minDPhiBremKFPos < 0.1) &&
      ...
      if (debug)
         cout << "runConvBremFinder - Found a track and close BREM " 
